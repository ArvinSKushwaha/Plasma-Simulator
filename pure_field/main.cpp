...
constexpr SizeTuple SIM_RES = {3, 3, 3}; // dimensionless max of 13500 elements
...
int main()
{
    ...
    for(int i = 0; i < SIM_RES.x; ++i)
    {
        for(int j = 0; j < SIM_RES.y; ++j)
        {
            ...
            {
                if(i == 0 && sqrt(pow(j - SIM_RES.y, 2) + pow(k - SIM_RES.z, 2)) < 5)
                {
                    massDensity.data[i][j][k] = total_mass/141.0;
                    velocity.data[i][j][k] = Vec3D(10000, 0, 0);
                    temperature.data[i][j][k] = 1000;
                }
                else
                {
                    massDensity.data[i][j][k] = 0;
                    velocity.data[i][j][k] = ZERO_VEC;
                    temperature.data[i][j][k] = 0;
                }
            }
        }
    }
                ...
                for(int i2 = 0; i2 < SIM_RES.x; ++i2)
                {
                    for(int j2 = 0; j2 < SIM_RES.y; ++j2)
                    {
                        for(int k2 = 0; k2 < SIM_RES.z; ++k2)
                        {
                            ...
                            if(r_prime.normSq() != 0)
                            {
                                // std::cout << r_prime.sstr() << std::endl;
                                magneticField.data[i1][j1][k1] +=  velocity(i2, j2, k2).cross(r_prime) * (total_charge / total_mass) * massDensity(i2, j2, k2) * (MU_0/(4 * PI * pow(r_prime.norm(), 3)));
                            }
                        }
                    }
                }
